...
#ifndef LINUX_MODE
# ifndef PSPFW30X
PSP_MODULE_INFO("PSPCAP32", 0x1000, 1, 1);
# else
PSP_MODULE_INFO("PSPCAP32", 0x0, 1, 1);
...
int 
PSPSDL_main(int argc, char *argv[])
{
	pspDebugScreenInit();
...
#ifdef PSP_ME
  if (psp_me_init()) {
    pspDebugScreenPrintf("Failed to initialize media engine !\n");
