...
static int loc_ly = 128;
  ...
  SDL_Event SDLEvent;    
  SDL_TextInputEvent *SDLTextEvent;    
  char buffer[512];
  ...
  if (SDL_PollEvent(&SDLEvent)) {
    ...
    if (SDLEvent.type==SDL_TEXTINPUT) {
    	SDLTextEvent = ((SDL_TextInputEvent*)(&SDLEvent));
        sprintf(buffer,"SDL Text=%s",SDLTextEvent->text);
        and_kbd_enter_data(SDLTextEvent->text[0]);
		Event = -1;
    }
    else
        and_kbd_enter_data(0);
    	
    if (SDLEvent.type==SDL_KEYDOWN||SDLEvent.type==SDL_KEYUP) {
		sprintf(buffer,"SDL Type: %X-SDL Scan code:%X",SDLEvent.type,Event);
    }
    switch (Event) {
      case SDL_SCANCODE_UP : Mask = PSP_CTRL_UP;
      break;
      case SDL_SCANCODE_DOWN : Mask = PSP_CTRL_DOWN;
      break;
      case SDL_SCANCODE_LEFT: Mask = PSP_CTRL_LEFT;
      break;
      case SDL_SCANCODE_RIGHT: Mask = PSP_CTRL_RIGHT;
      break;
      case SDL_SCANCODE_A    : Mask = PSP_CTRL_UPLEFT;
      break;
      case SDL_SCANCODE_E   : Mask = PSP_CTRL_UPRIGHT;
      break;
      case SDL_SCANCODE_LCTRL  : Mask = PSP_CTRL_DOWNLEFT;
      break;
      case SDL_SCANCODE_X : Mask = PSP_CTRL_DOWNRIGHT;
      break;
      case SDL_SCANCODE_2    : Mask = PSP_CTRL_SQUARE;
      break;
      case SDL_SCANCODE_3    : Mask = PSP_CTRL_CIRCLE;
      break;
      case SDL_SCANCODE_1     : Mask = PSP_CTRL_CROSS;
      break;
      case SDL_SCANCODE_4  : Mask = PSP_CTRL_TRIANGLE;
      break;
      case SDL_SCANCODE_5  : Mask = PSP_CTRL_LTRIGGER;
      break;
      case SDL_SCANCODE_6  : Mask = PSP_CTRL_RTRIGGER;
      break;
      case SDL_SCANCODE_RETURN     : Mask = PSP_CTRL_START;
      break;
      case SDL_SCANCODE_ESCAPE    : Mask = PSP_CTRL_SELECT;
      break;
