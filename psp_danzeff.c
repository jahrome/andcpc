...
danzeff_readInput(SceCtrlData pspctrl)
{
	...
	if (pspctrl.Buttons & PSP_CTRL_LEFT ) x -= 1;
	if (pspctrl.Buttons & PSP_CTRL_RIGHT) x += 1;
	if (pspctrl.Buttons & PSP_CTRL_UP   ) y -= 1;
	if (pspctrl.Buttons & PSP_CTRL_DOWN)  y += 1;
	...
	holding = pspctrl.Buttons & ~(PSP_CTRL_RTRIGGER|PSP_CTRL_LEFT|PSP_CTRL_RIGHT|PSP_CTRL_DOWN|PSP_CTRL_UP);
...
{
  char skin_path[128];
  strcpy(skin_path, CPC.cpc_homedir);
...
danzeff_load()
{
  ...
  if (psp_kbd_skin_first) {
	...
	{
    strcpy(tmp_filename, CPC.cpc_homedir );
