...
union TCounter Ton_Counter_A;
union TCounter Ton_Counter_B;
union TCounter Ton_Counter_C;
union TCounter Noise_Counter;
...
inline void Synthesizer_Logic_Q(void)
{
   ...
   Envelope_Counter.Hi++;
   //LUDO: BUGGY STUFF
   tmp = ((word)PSG.RegisterAY.EnvelopeHi << 8)|PSG.RegisterAY.EnvelopeLo;
   if (Envelope_Counter.Hi >= tmp) {
      Envelope_Counter.Hi = 0;
   }
}
...
void Synthesizer_Mono8(void)
{
   ...
   LoopCount.Re += LoopCountInit;
   CPC.snd_bufferptr[0] = 128 + Left_Chan / Tick_Counter; // write to mixing buffer
   CPC.snd_bufferptr++;
   Left_Chan = 0;
   if (CPC.snd_bufferptr >= pbSndBufferEnd) {
      cap32_swap_sound_buffer();
