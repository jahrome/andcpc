...
static void
render16bpp_gu_normal(void)
{
  SDL_Rect srcRect;
  SDL_Rect dstRect;

  srcRect.x = 0;
  srcRect.y = CPC.cpc_render_delta_y;
  srcRect.w = CPC_WIDTH;
  srcRect.h = CPC_HEIGHT-72;
  dstRect.x = 8;
  dstRect.y = 0;
  dstRect.w = PSP_SDL_SCREEN_WIDTH;
  dstRect.h = PSP_SDL_SCREEN_HEIGHT;

  psp_sdl_gu_stretch(&srcRect, &dstRect);
}

static void
render16bpp_gu_fit_width(void)
{
  SDL_Rect srcRect;
  SDL_Rect dstRect;
  ...
  srcRect.y = 0;
  srcRect.w = CPC_WIDTH;
  srcRect.h = CPC_HEIGHT;
  dstRect.x = 0;
  dstRect.y = 0;
  dstRect.w = PSP_SDL_SCREEN_WIDTH;
  dstRect.h = CPC_HEIGHT;

  psp_sdl_gu_stretch(&srcRect, &dstRect);
}

static void
render16bpp_gu_fit_height(void)
{
  SDL_Rect srcRect;
  SDL_Rect dstRect;

  srcRect.x = 17;
  srcRect.y = 35 + CPC.cpc_render_delta_y;
  srcRect.w = 350;
  srcRect.h = 200;
  dstRect.x = 0;
  dstRect.y = 0;
...
static void
render16bpp_gu_x125(void)
{
  SDL_Rect srcRect;
  SDL_Rect dstRect;

  srcRect.x = 0;
  srcRect.y = 27 + CPC.cpc_render_delta_y;
  srcRect.w = CPC_WIDTH;
  srcRect.h = 216;
  dstRect.x = 0;
  dstRect.y = 0;
...
static void
render16bpp_gu_x15(void)
{
  SDL_Rect srcRect;
  SDL_Rect dstRect;

  srcRect.x = 32;
  srcRect.y = 44 + CPC.cpc_render_delta_y;
  srcRect.w = 320;
  srcRect.h = 180;
  dstRect.x = 0;
  dstRect.y = 0;
...
render16bpp_smooth()
{
  if (CPC.cpc_render_mode == CPC_RENDER_NORMAL   ) render16bpp_gu_normal();
  else
  if (CPC.cpc_render_mode == CPC_RENDER_FIT_WIDTH) render16bpp_gu_fit_width();
  else
  if (CPC.cpc_render_mode == CPC_RENDER_FIT_HEIGHT) render16bpp_gu_fit_height();
  else
  if (CPC.cpc_render_mode == CPC_RENDER_X125)      render16bpp_gu_x125();
  else
  if (CPC.cpc_render_mode == CPC_RENDER_X15)       render16bpp_gu_x15();
}
